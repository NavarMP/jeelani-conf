...
import { Search, Filter, Copy, ChevronDown, ChevronRight, Trash2, Plus } from "lucide-react";
...
export default function ScheduleBuilder({ initialSessions = [], initialSpeakers = [] }: Props) {
  ...
  const [expandedParents, setExpandedParents] = useState<Set<string>>(new Set(initialSessions.filter(s => !s.parent_id).map(s => s.id)));
  ...
  const handleAddSession = async (parentId?: string) => {
    try {
      ...
      if (parentId && !expandedParents.has(parentId)) {
  ...
  // For top-level, we include it if IT matches OR if any of its children match
  const topLevel = allStage.filter(s => !s.parent_id).filter(parent => {
  ...
  const allChildren = allStage.filter(s => s.parent_id);

  const getFilteredChildren = (parentId: string) =>
    allChildren.filter(c => c.parent_id === parentId && matchSearch(c) && matchFilter(c))
      .sort((a, b) => new Date(a.start_time).getTime() - new Date(b.start_time).getTime());

  // All top-level sessions across all stages (for parent dropdown)
  const allTopLevel = sessions.filter(s => !s.parent_id);

  if (isLoading) return <div className="p-8 text-center text-[var(--admin-text-secondary)]">Loading schedule...</div>;
    ...
    <div className="bg-[var(--admin-surface)] rounded-xl border border-[var(--admin-border-subtle)] shadow-sm overflow-hidden">
      ...
      <div className="p-4 border-b border-[var(--admin-border)] flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        <div className="flex border border-[var(--admin-border)] rounded-lg overflow-hidden bg-[var(--admin-surface-alt)]">
          <button
            ...
            className={`px-4 py-2 text-sm font-medium transition-colors ${activeStage === 'stage1' ? 'bg-[var(--color-navy)] text-white' : 'text-[var(--admin-text-secondary)] hover:text-[var(--admin-text)]'}`}
          >
            Stage 1 (Main)
          </button>
          <button
            onClick={() => setActiveStage("stage2")}
            className={`px-4 py-2 text-sm font-medium transition-colors ${activeStage === 'stage2' ? 'bg-[var(--color-navy)] text-white' : 'text-[var(--admin-text-secondary)] hover:text-[var(--admin-text)]'}`}
          >
            Stage 2
          </button>
        </div>
      ...
      <div className="p-4 sm:p-6">
        <div className="space-y-4">
              ...
              <div key={session.id} className="space-y-2 relative">
                ...
                {isExpanded && sessionChildren.length > 0 && (
                  <div className="space-y-2 pl-2">
                    <p className="text-xs font-semibold text-[var(--admin-text-muted)] uppercase tracking-wider ml-6 mt-2">Sub-programs ({sessionChildren.length})</p>
          ...
          <button onClick={() => handleAddSession()} className="w-full py-4 border-2 border-dashed border-[var(--admin-input-border)] rounded-lg text-sm font-medium text-[var(--admin-text-secondary)] hover:text-[var(--admin-text)] hover:bg-[var(--admin-hover)] hover:border-gray-400 transition-all">
...
function SessionCard({
  ...
  return (
    <div className={`relative group ${isChild ? 'ml-6 border-l-4 border-[var(--color-turquoise)]/30 pl-2' : ''}`}>
      <div className={`flex flex-col lg:flex-row gap-4 p-4 border rounded-lg relative ${isChild ? 'bg-[var(--color-turquoise)]/5 border-[var(--color-turquoise)]/20' : 'bg-[var(--admin-surface)] border-[var(--admin-border)]'}`}>
        ...
        <div className="flex-1 space-y-3">
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
            <div className="flex-1">
              <label className="text-xs font-semibold text-[var(--admin-text-secondary)] uppercase tracking-wider">Title (EN)</label>
              <input
                type="text"
                value={session.title || ''}
                onChange={(e) => onUpdate(session.id, 'title', e.target.value)}
                className="w-full mt-1 p-1.5 border border-[var(--admin-input-border)] bg-[var(--admin-input-bg)] text-[var(--admin-text)] rounded text-sm outline-none focus:border-[var(--color-turquoise)] font-bold"
              />
            </div>
            <div className="flex-1">
              <label className="text-xs font-semibold text-[var(--admin-text-secondary)] uppercase tracking-wider">Title (ML)</label>
              <input
                type="text"
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
            <div className="w-full sm:w-32">
              <label className="text-xs font-semibold text-[var(--admin-text-secondary)] uppercase tracking-wider">Type</label>
              <select
            ...
            {!isChild && (
              <div className="flex-1">
                <label className="text-xs font-semibold text-[var(--admin-text-secondary)] uppercase tracking-wider">Slug</label>
                <input
                  type="text"
                  value={session.slug || ''}
                  onChange={(e) => onUpdate(session.id, 'slug', e.target.value)}
                  className="w-full mt-1 p-1.5 border border-[var(--admin-input-border)] bg-[var(--admin-input-bg)] rounded text-sm outline-none focus:border-[var(--color-turquoise)] text-[var(--admin-text-secondary)]"
                />
              </div>
            )}
            {isChild && (
              <div className="flex-1">
                <label className="text-xs font-semibold text-[var(--admin-text-secondary)] uppercase tracking-wider">Parent Group</label>
                <select
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full mt-1 p-2 border border-[var(--admin-input-border)] bg-[var(--admin-input-bg)] text-[var(--admin-text)] rounded text-sm outline-none focus:border-[var(--color-turquoise)] resize-none"
            />
          </div>

          <div>
            <label className="text-xs font-semibold text-[var(--admin-text-secondary)] uppercase tracking-wider">Speakers</label>
            <div className="mt-1 flex flex-wrap gap-2 items-center">
              {session.speakers?.map((speaker: any) => (
                <span key={speaker.id} className="px-2 py-1 bg-[var(--color-turquoise)]/10 text-[var(--color-turquoise)] border border-[var(--color-turquoise)]/20 rounded-md text-xs font-medium flex items-center gap-1">
                  {speaker.name}
                  <button
                    type="button"
        ...
        <div className="absolute right-3 top-3 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-2">
          <button type="button" onClick={() => onDuplicate(session)} className="p-1.5 bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-md text-[var(--admin-text-secondary)] hover:text-[var(--color-turquoise)] transition-colors cursor-pointer" title="Duplicate Session">
            <Copy className="w-4 h-4" />
          </button>
          <button type="button" onClick={() => onDelete(session.id)} className="p-1.5 bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-md text-[var(--admin-text-secondary)] hover:text-red-500 transition-colors cursor-pointer" title="Remove Session">
